followeUnfollow=document.querySelectorAll("[id^='followUnfollowHref']");
    ...
    function regAnswer(userId) {
        if (request.readyState === 4 && request.status === 200) {
            ...
            if (user.isFollowed ===1) {
                document.getElementById("followUnfollowSpan"+userId).className="userItemSubscribeS2";
                document.getElementById("followUnfollowSpan"+userId).textContent=jTextUnSubscribe;
            } else {
                document.getElementById("followUnfollowSpan"+userId).className="userItemSubscribeS1";
